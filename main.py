...
class Application():
    def __init__(self,master,*args,**kwargs):
        ...
        self.left=Frame(master,width=700,height=768,bg='orange')
        self.left.pack(side=LEFT)

        self.right = Frame(master, width=666, height=768, bg='yellow')
        self.right.pack(side=RIGHT)


        #components
        self.heading=Label(self.left,text="INVENTORY MANAGEMENT SYSTEM",font=('arial 26 bold'),fg='white',bg='black')
        self.heading.place(x=0,y=0)

        self.date_l=Label(self.right,text="Today's Date: "+str(date),font=('arial 14 bold'),bg='gray',fg='white')
        self.date_l.place(x=0,y=0)

        self.time_l = Label(self.right, text="Current Time: " + str(localtime), font=('arial 14 bold'), bg='gray',fg='white')
        self.time_l.place(x=0, y=30)
    ...
    def ajax(self,*args,**kwargs):
        self.conn = mysql.connector.connect(host='localhost',port=3306,database='inventory_system',user='root',password='')
